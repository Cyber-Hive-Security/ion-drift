...
async fn try_bootstrap(
    ...
    OsRng.try_fill_bytes(&mut kek_bytes)
        .map_err(|e| anyhow::anyhow!("OS RNG failed generating KEK: {e}"))?;
...
fn cache_kek(data_dir: &Path, key_path: &str, kek: &Key<Aes256Gcm>) -> anyhow::Result<()> {
    ...
    OsRng.try_fill_bytes(&mut nonce_bytes)
        .map_err(|e| anyhow::anyhow!("OS RNG failed generating nonce: {e}"))?;
...
pub fn derive_kek_from_password(password: &str, data_dir: &Path) -> anyhow::Result<BootstrapResult> {
    ...
    let salt: [u8; 16] = if salt_path.exists() {
        ...
        let mut s = [0u8; 16];
        OsRng.try_fill_bytes(&mut s)
            .map_err(|e| anyhow::anyhow!("OS RNG failed generating salt: {e}"))?;
