...
//! are serialized through a `tokio::sync::Mutex<Connection>` to match
//! SQLite's single-writer model. Long queries run via `spawn_blocking` so
//! they don't block the tokio runtime.
...
impl SqliteBackend {
    ...
    pub async fn open(data_dir: PathBuf, module_name: &str) -> Result<Self, StorageError> {
        ...
        // Use spawn_blocking because rusqlite::Connection::open blocks.
...
impl StorageBackend for SqliteBackend {
    fn write_any(
        ...
        Box::pin(async move {
            ...
            // Run the closure under a blocking-compatible scope. We don't use
            // spawn_blocking here because we hold a MutexGuard that isn't
            // Send across spawn; rusqlite operations are fast enough for
            // typical module workloads. Modules with long queries should
            // take care.
            let any = f(&mut *guard);
            Ok(any)
        })
