...
impl ModuleContext {
    ...
    /// `Default::default()` if the section is missing or invalid.
    ...
    /// Metrics registration handle. See [`MetricsHandle`].
...
pub struct EventHandle {
    ...
    declared_publish: Arc<Vec<EventKind>>,
    declared_subscribe: Arc<Vec<EventKind>>,
}
...
impl EventHandle {
    ...
    pub fn publish(&self, event: DriftEvent) -> Result<(), EventError> {
        ...
        if !self.declared_publish.contains(&kind) {
            return Err(EventError::NotDeclared(kind));
        }
        // broadcast::Sender::send returns Err if there are no receivers; that
        // is fine and not a real error — events are advisory.
    ...
    pub fn subscribe(&self) -> EventReceiver {
        EventReceiver {
            ...
            filter: self.declared_subscribe.clone(),
        }
    }
}

/// Filtered receiver — only yields events declared in the module's subscribe
/// set. Other events are silently skipped by the recv loop.
pub struct EventReceiver {
    inner: broadcast::Receiver<DriftEvent>,
    filter: Arc<Vec<EventKind>>,
}

impl EventReceiver {
    /// Await the next event matching the module's subscribe set.
    ///
    /// Returns `Err(EventError::Lagged(n))` if the module missed events and
    /// `Err(EventError::Closed)` if the bus was closed.
    pub async fn recv(&mut self) -> Result<DriftEvent, EventError> {
        loop {
            match self.inner.recv().await {
                Ok(event) => {
                    if self.filter.contains(&EventKind::of(&event)) {
                        return Ok(event);
                    }
                    // Skip events this module didn't subscribe to.
                }
...
///
/// v1.0 is intentionally minimal: modules can register counters, gauges, and
/// histograms by name. The host exposes these via the module health aggregator
/// and, when Drift gains a Prometheus endpoint, automatically via `/metrics`.
#[derive(Clone, Default)]
pub struct MetricsHandle {
    // Opaque placeholder for v1.0 — the registry concrete lives in -host.
    // A future minor bump will add counter/gauge/histogram registration
    // methods here once the host-side metrics registry lands.
    _placeholder: (),
}
